...
def event_dungeon_start(cla, where):
...
def event_dungeon_ready(cla, where):
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                if jadong_spl_[0] == "사냥":
                    ...
                    if jadong_spl_[0] == "일반" or jadong_spl_[0] == "특수":
                        ...
                        if jadong_spl_[1] == "이벤트":
                            full_path = "c:\\my_games\\zenonia\\data_zeno\\imgs\\dungeon\\juljun_title\\event_title.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def jadong_arrive_check(cla, where):
    ...
    from function import click_pos_2, click_pos_reg, imgs_set_, drag_pos
    from jadong_zeno import jadong_juljun_attack_gold_check
    from potion_zeno import juljun_potion_check, juljun_maul_potion
    try:
